...
class T5FlashHandler(FlashHandler):
            ...
            if erase_size-i > 0x10000:
                ...
                while cnt > 0 and not ret:
                    if self._flash_cfg.flash_size >= 256 * 1024 * 1024:
                        ret = self.ser_handle.erase_custom_size(start_addr+i,
                                                                0xdc)
                    else:
                        ret = self.ser_handle.erase_custom_size(start_addr+i,
                                                                0xd8)
                    cnt -= 1
                if not ret:
                    self.logger.error(f"Erase block(64K) error: {fmt_addr}")
                    self.progress.close()
                    return False
                self.progress.update()
                i += 0x10000
                ...
                while cnt > 0 and not ret:
                    ...
                    cnt -= 1
                if not ret:
                    self.logger.error(f"Erase sector(64K) error: {fmt_addr}")
                    self.progress.close()
                    return False
                self.progress.update()
                i += 0x1000
    ...
    def write(self):
        ...
        if start_addr & 0xfff:
            self.logger.debug("write align start ...")
...
{start_addr:08x} fail.")
        ...
        if end_addr & 0xfff:
            self.logger.debug("write align end ...")
            if not self.ser_handle.align_sector_address_for_write(
                                end_addr, False, wbuf,
                                flash_size):
        ...
        while i < file_len:
            if self.stop_flag:
                self.progress.close()
                return False
            self.logger.debug(f"write at {(i+start_addr):08x} ...")
